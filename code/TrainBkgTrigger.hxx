//Code generated automatically by TMVA for Inference of Model file [TrainBkgTrigger.h5] at [Wed Sep  6 21:26:49 2023] 

#ifndef TMVA_SOFIE_TRAINBKGTRIGGER
#define TMVA_SOFIE_TRAINBKGTRIGGER
...
namespace TMVA_SOFIE_TrainBkgTrigger{
namespace BLAS{
...
Session(std::string filename ="") {
   if (filename.empty()) filename = "TrainBkgTrigger.dat";
   std::ifstream f;
   f.open(filename);
   if (!f.is_open()){
   ...
   {
      float * data = TMVA::Experimental::SOFIE::UTILITY::UnidirectionalBroadcast<float>(tensor_dense3bias0,{ 1 }, { 32 , 1 });
...
//--------- Gemm
   ...
   std::vector<float> ret (tensor_dense3Sigmoid0, tensor_dense3Sigmoid0 + 32);
   return ret;
}
};
} //TMVA_SOFIE_TrainBkgTrigger

#endif  // TMVA_SOFIE_TRAINBKGTRIGGER
